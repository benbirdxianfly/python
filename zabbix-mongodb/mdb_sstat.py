...
class MGdb(object):
    ...
    def _get_result(self, iphost=None, port=None, username=None, password=None):
        try:
            ...
            stdo_str = stdo
            #print stdo_str
            stdo_str = stdo_str.replace("NumberLong(", "").replace(")", "").replace("ISODate(", "").replace("Timestamp(","").replace(", 1","")
            # print stdo_str
...
def main():
    ...
    parser.add_option("-H", "--host",
                      ...
                      default='192.168.1.207', help="Connect to memcached host.")

    parser.add_option("-p", "--port",
                      action="store", dest="port", type="string",
                      default='192.168.1.207-27017', help="the port for mongodb, for example: 27017")
    ...
    tmpstr = options.port.split("-")
    # print tmpstr
    hostname = tmpstr[0]
    port = int(tmpstr[1])
    # hostname = options.port.s
    # port = options.port
    ...
    monitor_obj = MGdb(iphost=hostname, port=port, username=username, password=password, debug=options.debug,
                       force=options.force)
