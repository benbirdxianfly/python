...
class Main:
 ...
 #       print('  ==> 点击搜索 ==>  ')
        ...
        # 查找成功
        # elif self._adb.find_nodes_by_text('W.'):
        #     #self._adb.click(0)
        #     print( phone + 'W')
        #     time.sleep(1)
...
#   在此处做判断   进行处理有的加好友不需要点击发送
            

            self._adb.refresh_nodes()
            if self._adb.find_nodes_by_text('W'):
                 print("=======sum++++++++"+ phone + 'W')
                 time.sleep(1)
        ...
        elif self._adb.find_nodes_by_text('该用户不存在'):
            print('  <== 该用户不存在 或 帐号异常 <==  ')
            ...
            self._adb.adb_put_back()
        elif  self._adb.find_nodes_by_text('被搜帐号状态异常，无法显示'):
            print('  <== 被搜帐号状态异常，无法显示 <==  ')
            self.push('failed', phone + '      被搜帐号状态异常，无法显示')
            self._adb.adb_put_back()
